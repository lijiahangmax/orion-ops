<template>
  <div class="app-build-statistics-record-container">
    ...
    <!-- 构建明细 -->
    <div class="app-build-statistics-container gray-box-shadow">
      <div class="statistic-tabs-wrapper" v-show="profileId && appList.length">
        <a-tabs v-model="activeTab" :animated="false" @change="chooseStatisticsTab">
          ...
          <a-tab-pane key="1" tab="视图">
            <AppBuildStatisticsViews ref="view"/>
          </a-tab-pane>
          <!-- 指标 -->
          <a-tab-pane key="2" tab="指标">
            <AppBuildStatisticsMetrics ref="metrics"/>
          </a-tab-pane>
          <!-- 图表 -->
          <a-tab-pane key="3" tab="图表">
            <AppBuildStatisticsCharts ref="chart"/>
          </a-tab-pane>
        </a-tabs>
      </div>
...
export default {
  ...
  components: {
    AppBuildStatisticsMetrics: () => import('@/components/app/AppBuildStatisticsMetrics'),
    AppBuildStatisticsViews: () => import('@/components/app/AppBuildStatisticsViews'),
    AppBuildStatisticsCharts: () => import('@/components/app/AppBuildStatisticsCharts')
  },
  data() {
    return {
      appLoading: false,
